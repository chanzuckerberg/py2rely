"""FastAPI application and uvicorn launcher for relion-ui."""
...
from py2rely.relionui.models import FileList, JobDetail, PipelineGraph
...
from py2rely.relionui.watcher import PipelineWatcher
...
app = FastAPI(title="relion-ui", version="0.1.0", lifespan=_lifespan)
...
def launch(
    ...
    if not pipeline_star.exists():
        print(
            f"\n[relion-ui] No 'default_pipeline.star' found in {PROJECT_DIR}\n"
            "Make sure you run 'py2rely ui' from inside a RELION project directory.\n"
        )
        raise SystemExit(1)

    url = f"http://{host}:{port}"
    print(f"\n[relion-ui] Running at {url}")

    if host in ("0.0.0.0", "::"):
        print(
            f"[relion-ui] If on a remote host, forward with:\n"
